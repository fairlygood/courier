...
# =========================================================
# Function: copy_fonts
# Purpose: Copy font files from assets/fonts/ to build/generated/fonts/
# Params: $1 project root; $2 build/generated dir
# Returns: none
# =========================================================
copy_fonts() {
    local project_root="$1"
    ...
    local fonts_dir="$project_root/assets/fonts"
    [[ ! -d "$fonts_dir" ]] && return

    local target_dir="$build_generated_dir/fonts"
    mkdir -p "$target_dir"
    ...
    if [[ $count -gt 0 ]]; then
        write_color_output "Copied $count font(s) to: $target_dir" "Green"
    fi
}
...
main() {
    ...
    copy_fonts "$project_root" "$gen_dir"
